            ...
            <div class="information_hd ">
                <ul>
                    ...
                    <div class="information_time">
                       ...
                       <span class="action">
                        <a rel="nofollow" href="javascript:setbg('收藏信息',538,270,'<?=$mymps_global['SiteUrl']?>/box.php?part=shoucang&infoid=<?=$info['id']?>')">收藏</a>
                        <a href="javascript:setbg('置顶信息',538,248,'<?=$mymps_global['SiteUrl']?>/box.php?part=upgrade&id=<?=$info['id']?>');">置顶</a>
                        <a rel="nofollow" href="javascript:setbg('删除信息',538,270,'<?=$mymps_global['SiteUrl']?>/box.php?part=delinfo&id=<?=$info['id']?>')" title="一旦删除将不能恢复！">删除</a></li>
                        <a rel="nofollow" href="<?=$mymps_global['SiteUrl']?>/<?=$mymps_global['cfg_postfile']?>?action=edit&id=<?=$info['id']?>" target="_blank">修改</a>
                        <a class="report" href="javascript:setbg('举报信息',470,300,'<?=$mymps_global['SiteUrl']?>/box.php?part=report&id=<?=$info['id']?>&infotitle=<?=$info['title']?>');">举报</a>
                       </span>
                    </div>
                </ul>
            </div>
            <div class="clear"></div>
<div class="extra_contact <? if($info['info_level'] == 2) { ?>tuijian<?php } ?>">
            <? if($info['image']) { ?>
            <div class="extra">
                <div class="zoombox">
            ...
            <div class="contact">
                <ul>
                    <li><span>区域：</span><?=$info['areaname']?> <?=$info['streetname']?></li>
                    <?php if(is_array($info['extra'])){foreach($info['extra'] as $mymps) { ?>                    <? if($mymps['value']) { ?>
                    <li style="<? if(in_array($mymps['title'],array('价格','租金'))) { ?>color:red; font-weight:bold;<?php } ?>"><span><?=$mymps['title']?>：</span><? if(in_array($mymps['title'],array('0元','0元/月','0万元','0元/平米','0元/平米/天','0平米'))) { ?>面议<?php } else { ?><?=$mymps['value']?><?php } ?></li>
                    <?php } ?>
                    <?php }} ?>
                </ul>
            </div>
...
<div class="bd_right">
        	<div class="boxer_hd cfix userhd"><div class="clear5"></div><? if(!$member['prelogo']) { ?><img src="<?=$mymps_global['SiteUrl']?>/images/noavatar.gif" title="<?=$member['tname']?>" alt="<?=$member['tname']?>" width="120" height="120"><div class="clear"></div><?php } else { ?><img src="<?=$mymps_global['SiteUrl']?><?=$member['prelogo']?>" title="<?=$member['tname']?>" alt="<?=$member['tname']?>" width="180" height="150"><div class="clear5"></div><?php } ?></div>
            
            <div class="cfix user">
            	<ul>
                <? if($info['contactview'] == 1) { ?>
                	<? if($info['userid']) { ?>
                    <li><span>发布人</span><?=$info['userid']?></li>
                    <?php } ?>
                    <li><span>联系人：</span><?=$info['contact_who']?> </li>
                    ...
                    <?php } else { ?>
                        <li><span>联系电话：</span><a target="_blank" href="<?=$info['posthistory']?>" title="点击查看发贴记录"><font class="tel"><?=$info['telephone']?></font></a></li>
                        <? if($info['qq']) { ?><li><span>QQ号码：</span><?=$info['qq']?></li><?php } ?>
                        <? if($info['email']) { ?><li><span>电子邮箱：</span><?=$info['email']?></li><?php } ?>
                    <?php } ?>
                <?php } else { ?>
                    <li>&nbsp;&nbsp;该信息已过期，联系方式自动隐藏</li>
                <?php } ?>
                </ul>
            </div>
<div class="boxer_hd cfix">您感兴趣的信息</div>
<div class="boxer_bd cfix">
<ul><?php if(is_array($latest_info)){foreach($latest_info as $mymps) { ?><li><a style="<? if($mymps['ifred'] == 1) { ?>color:red;<?php } ?> <? if($mymps['ifbold'] == 1) { ?>font-weight:bold;<?php } ?>" href="<?=$mymps['uri']?>" target="_blank" title="<?=$mymps['title']?>"><? if($mymps['img_path']) { ?><font color=green>[图]</font> <?php } ?><?=$mymps['title']?></a></li>
<?php }} ?>
</ul>
</div>
...
<div class="boxer_hd cfix">热门分站<?=$info['catname']?>信息</div>
<div class="boxer_bd cfix noborder short">
<ul>
                <?php if(is_array($hotcities)){foreach($hotcities as $mymps) { ?><li><a href="<?=$mymps['domain']?><?=$info['caturi']?>" target="_blank" title="<?=$mymps['cityname']?><?=$info['catname']?>信息"><?=$mymps['cityname']?><?=$info['catname']?>信息</a></li>
<?php }} else {{ ?>
<li>暂无热门分站信息！</li>
<?php }} ?>
</ul>
